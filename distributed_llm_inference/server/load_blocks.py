    ...
    with safetensors.safe_open(file, framework="pt", device="cpu") as f:
        for key, value in f.items():
            if key.startswith(block_prefix):
                sharded_state_dict[key[len(block_prefix):]] = value
        
    return sharded_state_dict


def get_block_state_dict(repo: str, block_idx: int, cache_dir: str | None = None):
    for pattern in INDEX_FILE_PATTERNS:
        index_file = cached_file(repo, pattern, cache_dir=cache_dir)
    ...
    for file in weight_files:
        sharded_state_dict = get_sharded_block_state_from_file(file, prefix)
...
def load_block(model_name: str, block_idx: int, cache_dir: str | None = None) -> LlamaBlock:
    config = AutoConfig.from_pretrained(model_name)
    state_dict = get_block_state_dict(model_name, block_idx, cache_dir=cache_dir)
